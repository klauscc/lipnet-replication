...
import cv2
def my_model():
    input_dim = (75, 50, 100, 3) 
    output_dim = 2
    max_label_length = 50
    speaker = 10
    weights_path = './data/checkpoints_grid/grid_vsa_speaker_22.hdf5'
    lipnet_weights = './data/checkpoints_grid/lipnet_res3d_weights_multiuser_with_auth_2017_08_31__12_30_02-0-44.0345230103-0.349565677966.hdf5'
    pos_n = 25
    data_generator = GRIDSinglePersonAuthentication(auth_person=speaker, pos_n=pos_n)
    model,test_func = lipnet_res3d(input_dim=input_dim, output_dim=max_label_length, weights = lipnet_weights, speaker= speaker, viz=True)
    # model.load_weights(weights_path) 
    model.summary() 

    lip_path = './data/GRID/lips/s11/bwwm6n'
    mini_batch_lips = data_generator.gen_batch(begin=0, 
            ...
            auth_person=speaker,
            scale=1/255.) 
    inputs,outputs = mini_batch_lips
    # print (outputs)
    # Swap softmax with linear, only needed when visualing softmax layer
    layer_idx = utils.find_layer_idx(model, 'y_pred')
    # model.layers[layer_idx].activation = activations.linear
    # model = utils.apply_modifications(model)
    # inputs = inputs[0] 

    visualize_saliency_3Dcnn(model, layer_idx, filter_indices=1, seed_input=inputs[ 'inputs'] ,
                             original_img=inputs[ 'inputs'][0]  *255,backprop_modifier= 'guided')
# def test():
...
class ActivationMaximization3D(Loss):
    ...
    def __init__(self, layer, filter_indices):
        ...
        """
        super(ActivationMaximization3D, self).__init__()
    ...
    def build_loss(self):
        ...
        loss = 0.
        T = 75
        #layer_output shape: (b, t, c)
        for t in range(75):
            loss += -K.max(layer_output[0,t,:] ) 
...
def visualize_saliency_3Dcnn(model, layer_idx, filter_indices, seed_input,original_img,
    ...
    visualize_saliency_with_losses(model.input[0] , losses, seed_input,original_img, grad_modifier)
