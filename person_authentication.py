...
train_steps = gridPerson.train_num // batch_size // 10
print ( "train_steps : {}".format(train_steps) )  // 5
...
weight_savepath = ''
checkpointer = ModelCheckpoint(filepath='./data/checkpoints/grid_person_authentication_my_model1-{epoch:02d}-{val_loss:.2f}-{val_acc:.4f}.hdf5',save_best_only=True,save_weights_only=True, verbose=1)
net = My_model1().build_net(input_dim = input_dim)  
