...
class OurLipPersonAuthentication(GRIDBaseDataset):
    def __init__(self, num_classes = 200, *args, **kwargs):
        GRIDBaseDataset.__init__(self, *args, **kwargs)

        self.num_classes = num_classes
        people = np.arange(1,num_classes+1)
        ...
        self.val_paths = self.get_sample_paths(self.lip_sequence[test_upper:train_n]  ) 
        self.val_num = len(self.val_paths) 
        print self.train_num, self.val_num
    ...
    def getLipDirOfPerson(self, i):
        return "{}/5sec10/{}".format(self.data_root, i)
    ...
    def get_sample_paths(self, paths, timespec=1, stride=None):
        ...
        per_sample = []
        for path in paths:
            for i in range(2, self.timespecs+1, stride):
                if timespec == 1:
                    img = '{}/IMG_{:04d}.jpg'.format(path, i)
    ...
    def read_data(self,per_sample_path):
        def get_y_indice(one_path):
            return int(os.path.basename(os.path.dirname(os.path.dirname(one_path) ) ) ) -1
...
if __name__ == "__main__":
    gridPersonDatabase = OurLipPersonAuthentication( debug = True, data_dir= './data/ourlip')
    batch_size=100
